<?php
################
## Скрипт для формирования списка игр p2p секции
## 
##
...
$stmt = pdo()->prepare("SELECT * FROM p2p_games WHERE (gamer1_name LIKE '$name' OR gamer2_name LIKE '$name') AND game_delete!=1 AND (tag LIKE '$tag' OR tag IS NULL) AND last_update LIKE '%$dategame%' ORDER BY last_update DESC LIMIT $offset, $size_page");
$stmt->execute();
