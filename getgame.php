...
if ($list === "p2p") {
    $stmt = pdo()->prepare("SELECT * FROM p2p_games WHERE privateStartGame = 0 AND last_update >= now() - interval 1 hour ORDER BY last_update DESC");
    $stmt->execute();
                ...
                echo '
                    <div class="container">
                        <div class="row justify-content-center align-items-center g-2">
                            <div class="col">' . $row['gamer1_name'] . ' VS ' . $row['gamer2_name'] . '</div>
                            <div class="col">' . $row['gamer2_name'] . '</div>
                            <div class="col">' . $row['require1'] . ' : ' . $row['require2'] . '</div>
                        </div>
                    </div>
                    ';
            else {
?>
                ...
                <div class="container">
                    <div class="row justify-content-center align-items-center g-2">
                        ...
                        <div class="col"><a href="<?php if (!$row['gamer2_name']) {echo $row['key'];} ?>">Присоединиться к игре</a></div>
...
if ($deleteGame == true && $_SESSION['user_role'] == 1) {
    ...
    print_r($stmt);
    die;
} else { // Продолжаем выполнение основного скрипта
    ...
    <nav>
      <ul class="pagination justify-content-center">
        ...
        <li class="<?php if ($pageno <= 1) {
            ...
            <a class="page-link" onclick='javascript:getgame(pageno=<?php if ($pageno <= 1) {
                                                                        echo "#";
                                                                    } else {
                                                                        echo ($pageno - 1);
                                                                    } ?>);' href="#">Назад</a>
        ...
        <li class="<?php if ($pageno >= $total_pages) {
            ...
            <a class="page-link" onclick='javascpript:getgame(pageno=<?php if ($pageno >= $total_pages) {
                                                                            echo "#";
                                                                        } else {
                                                                            echo ($pageno + 1);
                                                                        } ?>)' href="#">Вперед</a>
